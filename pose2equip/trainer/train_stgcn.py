...
# Loss
# =========================
def _to_equip_point_repr_torch(obj: torch.Tensor) -> torch.Tensor:
    """Normalize object points to [..., equip, point, 3] = [..., 4, 2, 3]."""
    if obj.ndim >= 3 and obj.shape[-3:] == (4, 2, 3):
        return obj

    # Backward compatibility: [..., 8, 3] -> [..., 4, 2, 3]
    if obj.ndim >= 2 and obj.shape[-2:] == (8, 3):
...
def mpjpe(pred, gt):
    ...
    Supports [..., 4, 2, 3] and [..., 8, 3] inputs.
    """
    pred = _to_equip_point_repr_torch(pred)
    gt = _to_equip_point_repr_torch(gt)
...
def length_variance_loss(pred_obj):
    ...
    pred_len_flat = pred_len.reshape(-1, 4)

    left_ski_len = pred_len_flat[:, 0]
    right_ski_len = pred_len_flat[:, 1]
    left_pole_len = pred_len_flat[:, 2]
    right_pole_len = pred_len_flat[:, 3]
...
def equipment_segment_lengths(obj: torch.Tensor) -> torch.Tensor:
    ...
    Supports [..., 4, 2, 3] and [..., 8, 3].
    Returns shape [..., 4].
    """
    obj = _to_equip_point_repr_torch(obj)
...
def _to_flat_n83_np(obj: np.ndarray) -> np.ndarray:
    """Normalize numpy object tensor to [N, 8, 3] for metric computation."""
    arr = np.asarray(obj)
    if arr.shape[-3:] == (4, 2, 3):
...
def evaluate_pose_metrics(pred_obj: np.ndarray, gt_obj: np.ndarray) -> Dict[str, float]:
    ...
    Args:
        pred_obj: (..., 4, 2, 3) or (..., 8, 3) predicted object keypoints
        gt_obj: (..., 4, 2, 3) or (..., 8, 3) ground truth object keypoints

    Returns:
        Dict with keys: mpjpe, pa_mpjpe, mpjpe_left_ski, mpjpe_right_ski, mpjpe_left_pole, mpjpe_right_pole
    """
    pred_obj = _to_flat_n83_np(pred_obj)
    gt_obj = _to_flat_n83_np(gt_obj)

    # MPJPE: direct Euclidean distance
    mpjpe_val = np.linalg.norm(pred_obj - gt_obj, axis=2).mean()  # (N, 8) -> scalar
...
class Pose2Equip_STGCN_Trainer(LightningModule):
    ...
    def _build_object_gt(
        ...
        ski_obj = self._select_points(
            ski_gt, self.ski_gt_idx, "ski_gt"
        )  # [B, 4, 3] or [B, T, 4, 3]
        pole_obj = self._select_points(
            pole_gt, self.pole_gt_idx, "pole_gt"
        )  # [B, 4, 3] or [B, T, 4, 3]

        return torch.stack(
            [ski_obj, pole_obj], dim=-2
        )  # [B, T, 4, 2, 3] or [B, 4, 2, 3]
    ...
    def _shared_step(self, batch: Dict[str, Any], stage: str) -> torch.Tensor:
        ...
        pred_obj = out["object_3d"]  # [B, T, 4, 2, 3]
