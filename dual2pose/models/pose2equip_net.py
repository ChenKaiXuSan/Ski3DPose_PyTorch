...
class STGCNBaselineNet(nn.Module):
    ...
    def forward(self, human_3d: torch.Tensor):
        # human_3d: [B, J, 3] single frame or [B, T, J, 3] sequence
        # Output: [B, 8, 3] for single frame or [B, T, 8, 3] for sequence
        b = human_3d.shape[0]
        pose_feat = self.pose_encoder(human_3d)  # [B, hidden_dim] or [B, T, hidden_dim]

        if pose_feat.ndim == 2:  # Single frame mode [B, hidden_dim]
            pred_obj = self.equip_head(pose_feat).reshape(
                b, self.num_equip_kpts, 3
            )  # [B, 8, 3]
        else:  # Sequence mode [B, T, hidden_dim]
            t = pose_feat.shape[1]
            pred_obj = self.equip_head(pose_feat).reshape(
                b, t, self.num_equip_kpts, 3
            )  # [B, T, 8, 3]
